...
```mermaid
graph TD
    A[🖥️ Frente de Caixa - Angular] -->|HTTP POST /vendas| C(🌐 NGINX REVERSE PROXY)
    B[📊 Painel Admin - Angular] -->|HTTP GET/POST/PUT| C
    C --> D[☕ checkout-backend - Quarkus]
    D -->|Hibernate JPA| E[(🐘 infrastructure - PostgreSQL)]
